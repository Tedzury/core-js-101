...
function isCreditCardNumber(ccn) {
  return ccn
    ...
    .reduce((acc, curr, i) => {
      ...
      if (+curr * 2 > 9) return acc + (+curr * 2 - 9);
      return acc + +curr * 2;
    }, 0) % 10 === 0;
}
...
function getDigitalRoot(num) {
  ...
  const step = num.toString().split('').reduce((acc, curr) => acc + +curr, 0);
  return getDigitalRoot(step);
}
...
function getCommonDirectoryPath(pathes) {
  ...
  const result = pathesArr[0].filter((dir, i) => pathesArr.every((arr) => arr[i] === dir));
