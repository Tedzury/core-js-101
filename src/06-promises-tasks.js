...
function chainPromises(array, action) {
  const promises = [];
  return Promise.resolve(array).then((promise) => {
    promise.forEach((item) => {
      item.then((val) => promises.push(val)).catch((err) => new Error(err));
    });
  }).then(() => promises.reduce((acc, e) => action(acc, e))).catch((err) => new Error(err));
}
